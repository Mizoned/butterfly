  ...
  const submitHandler = async () => {
      await scheduleStore.cancelSchedule()
        .then(async () => {
          ...
          await schedulesStatisticsStore.getCanceledStatistics();
        })
        .catch((error => {
...
<template>
  <Dialog v-model:visible="scheduleStore.isOpenCancelScheduleDialog" :style="{ width: '450px' }" header="Подтверждение" :modal="true">
